    ...
    def save(self):
        ...
        if self.id:
            cursor.execute('''UPDATE users SET username=?, password=?, email=?, 
                           balance=?, is_admin=? WHERE id=?''',
                         (self.username, self.password, self.email, 
                          self.balance, self.is_admin, self.id))
        else:
            cursor.execute('''INSERT INTO users (username, password, email, balance, is_admin)
                          VALUES (?, ?, ?, ?, ?)''',
                         (self.username, self.password, self.email, 
                          self.balance, self.is_admin))
...
class Vehicle:
    ...
    def get_by_id(cls, vehicle_id):
        ...
        conn.close()
        return cls(*vehicle_data) if vehicle_data else None

    def save(self):
        conn = sqlite3.connect('auction.db')
        cursor = conn.cursor()
        if self.id:
            cursor.execute('''UPDATE vehicles SET make=?, model=?, year=?, mileage=?,
                           description=?, reserve_price=?, end_time=?, seller_id=?
                           WHERE id=?''',
                         (self.make, self.model, self.year, self.mileage,
                          self.description, self.reserve_price, self.end_time,
                          self.seller_id, self.id))
        else:
...
class Bid:
    ...
    def get_highest_for_vehicle(cls, vehicle_id):
        conn = sqlite3.connect('auction.db')
        cursor = conn.cursor()
        cursor.execute('''SELECT * FROM bids WHERE vehicle_id=? 
                       ORDER BY amount DESC LIMIT 1''', (vehicle_id,))
        bid_data = cursor.fetchone()
        conn.close()
        return cls(*bid_data) if bid_data else None

    def save(self):
        conn = sqlite3.connect('auction.db')
        cursor = conn.cursor()
        cursor.execute('''INSERT INTO bids (amount, time, bidder_id, vehicle_id)
                       VALUES (?, ?, ?, ?)''',
...
class Notification:
    ...
    def save(self):
        ...
        cursor = conn.cursor()
        if self.id:
            cursor.execute('''UPDATE notifications SET user_id=?, message=?, 
                           time=?, is_read=? WHERE id=?''',
                         (self.user_id, self.message, self.time, 
                          self.is_read, self.id))
        else:
            cursor.execute('''INSERT INTO notifications 
                           (user_id, message, time, is_read)
                           VALUES (?, ?, ?, ?)''',
